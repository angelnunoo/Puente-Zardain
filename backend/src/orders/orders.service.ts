...
export class OrdersService {
  ...
  async create(userId: string, payload: CreateOrderDto, ip?: string) {
    ...
    const products: OrderProduct[] = await this.ordersRepository.findProductsByIds(itemIds);
    ...
    let offerCost = 0;
    let offerName: string | undefined = undefined;
    ...
    const order = await this.prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (offerId && offerCost > 0) {
        await this.zardasService.redeemZardasInTransaction(
          tx,
          userId,
          offerCost,
          `Canje de oferta ${offerName || offerId} en pedido ${created.id.slice(0, 8)}`,
        );
