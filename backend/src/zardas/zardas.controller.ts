...
export class ZardasController {
  ...
  @Get(':userId/balance')
  getBalance(@Req() req: any, @Param('userId') userId: string) {
  ...
  @UseGuards(JwtAuthGuard)
  @Get(':userId/history')
  getHistory(@Req() req: any, @Param('userId') userId: string) {
    this.assertCanAccessUser(req.user, userId);
  ...
  @UseGuards(JwtAuthGuard)
  @Post(':userId/redeem')
  redeemZardas(@Req() req: any, @Param('userId') userId: string, @Body() body: { discountAmount: number; reason: string }) {
    this.assertCanAccessUser(req.user, userId);
  ...
  private assertCanAccessUser(user: { userId?: string; id?: string; role?: Role | string } | undefined, userId: string) {
    const requesterId = user?.userId ?? user?.id;
    if (!requesterId || (requesterId !== userId && user?.role !== Role.ADMIN)) {
      throw new ForbiddenException('No tienes permiso para consultar estas Zardas.');
    }
  }
}